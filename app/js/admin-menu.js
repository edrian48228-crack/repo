 ...
 * - FIX: PORTADA_URL se infiere automáticamente — funciona en cualquier repo.
 */
(function() {
  'use strict';

  // ── Inferir URL de portada automáticamente desde la URL actual ──
  // Funciona para cualquier repo: github.io/repo/app/ → github.io/repo/app/
  function inferPortadaUrl() {
    try {
      const loc = window.location;
      const path = loc.pathname;
      const appIdx = path.indexOf('/app/');
      if (appIdx !== -1) {
        return loc.origin + path.substring(0, appIdx + 5);
      }
  ...
  const PORTADA_URL = inferPortadaUrl();
  ...
  function ensureDesktopPortadaButton() {
    ...
    else if (nav && nav.parentNode) nav.parentNode.insertBefore(link, nav.nextSibling);
    else if (adminBox) adminBox.appendChild(link);
  }
  ...
  function protectOldLogoutFunctions() {
    ...
    window.openM = function(id) {
      if (id === 'm-admin-exit') { goPortada(); return false; }
      if (typeof originalOpenM === 'function') return originalOpenM.apply(this, arguments);
      return undefined;
    };
    window.doAdminLogout = function() { goPortada(); return false; };
  }
