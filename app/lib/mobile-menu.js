/* mobile-menu.js — Menú hamburguesa SOLO para dispositivos móviles (≤900px).
 * En desktop (>900px) no inyecta panel, no clona botones, no interfiere.
 * Reutiliza .hnav-wrap y los enlaces existentes.
 */
(function () {
  const BREAK = 900;
  ...
  function injectStyles() {
    ...
    const css = `
      .mtp-burger{
        ...
        background:transparent;border:1.5px solid var(--o,#ff6600);color:var(--o,#ff6600);
      ...
      .mtp-mobile-panel{
        ...
        width:min(86vw,340px);background:var(--bg2,#0e0e0e);
        border-left:1px solid var(--bdr,#2a2a2a);z-index:9999;
        transform:translateX(100%);transition:transform .28s ease;
        display:flex;flex-direction:column;
        padding:14px 14px calc(env(safe-area-inset-bottom,0px) + 28px);
        overflow-y:auto;-webkit-overflow-scrolling:touch;
        overscroll-behavior:contain;box-shadow:-10px 0 40px rgba(0,0,0,.5)
      }
      .mtp-mobile-panel.open{transform:translateX(0)}

      .mtp-mobile-head{
        display:flex;align-items:center;justify-content:space-between;
        margin-bottom:14px;padding-bottom:12px;border-bottom:1px solid var(--bdr,#2a2a2a)
      }
      .mtp-mobile-title{
        font-family:var(--fh,Nunito);font-weight:800;
        color:var(--o,#ff6600);font-size:15px;letter-spacing:.5px
      }
      .mtp-mobile-close{
        background:transparent;border:none;color:var(--t1,#fff);
        font-size:26px;cursor:pointer;line-height:1;padding:4px 10px;border-radius:8px
      }
      .mtp-mobile-close:hover{background:rgba(255,255,255,.06)}

      .mtp-mobile-panel .mtp-mobile-links a,
      .mtp-mobile-panel .mtp-mobile-links .hnav{
        display:block;padding:13px 14px;border-radius:10px;
        color:var(--t1,#fff)!important;text-decoration:none;
        font-family:var(--fh,Nunito);font-weight:700;font-size:14px;
        border:1px solid transparent;margin-bottom:4px;background:transparent
      }
      .mtp-mobile-panel .mtp-mobile-links a:hover,
      .mtp-mobile-panel .mtp-mobile-links .hnav:hover{
        background:rgba(255,102,0,.08);border-color:rgba(255,102,0,.3);
        color:var(--o,#ff6600)!important
      }
      .mtp-mobile-panel .mtp-mobile-links .hnav-free{
        background:linear-gradient(135deg,#ff2200,#ff6600);color:#fff!important
      }
      ...
      .mtp-mobile-actions button,
      .mtp-mobile-actions a{width:100%;text-align:center}
      ...
      @media (max-width:${BREAK}px){
        ...
        header#hdr .hdr-right > a:not(.mtp-burger),
        header#hdr .hdr-right > button:not(.mtp-burger){display:none!important}
        header#hdr .hdr-right{gap:6px}
      }

      @media (min-width:${BREAK + 1}px){
        .mtp-burger{display:none!important}
        .mtp-mobile-panel{display:none!important}
        .mtp-mobile-backdrop{display:none!important}
        header#hdr .hnav-wrap{display:flex!important}
        header#hdr .hdr-right > a,
        header#hdr .hdr-right > button{display:inline-flex!important}
      }
    `;
  ...
  function buildPanel() {
    ...
    const navLinks = document.querySelectorAll(
      'header#hdr .hnav-wrap .hnav, header#hdr .hnav-wrap a'
    );
    const linksHost = panel.querySelector('.mtp-mobile-links');
    navLinks.forEach((a) => {
      const c = a.cloneNode(true);
      c.addEventListener('click', closePanelFn);
      linksHost.appendChild(c);
    });

    const actions = document.querySelectorAll(
      'header#hdr .hdr-right > a, header#hdr .hdr-right > button'
    );
    const actHost = panel.querySelector('.mtp-mobile-actions');
    actions.forEach((b) => {
      if (b.classList.contains('mtp-burger')) return;
      const c = b.cloneNode(true);
      c.addEventListener('click', () => setTimeout(closePanelFn, 50));
      actHost.appendChild(c);
    });

    panel.querySelector('.mtp-mobile-close').addEventListener('click', closePanelFn);
  ...
  function injectBurger() {
    ...
    btn.addEventListener('click', openPanelFn);
